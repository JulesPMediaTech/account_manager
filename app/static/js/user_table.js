...
headers.forEach((header) => {
  header.addEventListener("click", () => {
    ...
    console.log(
      `Sorted by header "${header.textContent.trim()}" (col ${colIndex})`,
    );
    if (activeHeader) {
    ...
    activeHeader.classList.add("sorted-by-column");
    console.log(
      `Header ${activeHeader.textContent} has the class ${activeHeader.classList.value}`,
    );
  });
});
...
tableRows.forEach((row) => {
  row.addEventListener("click", () => {
    ...
    if (userId) {
      ...
      .then(data => {
        if (data.error) {
          // Access denied: Show modal and inject the server's error message without reloading
          const dialog = document.getElementById('js-dlg-warning-main');
          if (dialog) {
             dialog.querySelector('.js-dlg-warning-message-pane').innerHTML = data.error;
             dialog.showModal();
          }
        } else if (data.redirect) {
          // Success: proceed to the edit view
          
          window.location.href = data.redirect;
        }
      })
      .catch(err => console.error("Error forwarding user ID:", err));
    };
  });
});
